...
- `services.fail2ban` now supports systemd socket activation via `fail2ban.socket`. The socket is owned by a system group `fail2ban` with mode `0660` (previously owned by the `root` group and had mode `0600`).

- `services.prometheus.exporters.fail2ban` now runs as `DynamicUser` instead of the `fail2ban-exporter` user.
